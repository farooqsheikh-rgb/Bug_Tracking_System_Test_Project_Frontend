...
export default function AddBugDialog({
  open,
  onClose,
  projectId,
  onBugCreated,
  ...
  const fetchProjectMembers = useCallback(async () => {
    try {
      ...
      if (result.success) {
        const developers = result.data.filter(
          (user: User) => user.user_type === "developer"
        );
      ...
      if (assignedUsers.length > 0) {
        for (const user of assignedUsers) {
          ...
          if (!assignResult.success) {
            console.error(
              `Failed to assign user ${user.name} to bug:`,
    ...
    >
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <Typography
            sx={{ fontWeight: 500, fontSize: "27.35px", color: "#000000" }}
          >
            Add new
          </Typography>
          <FormControl size="small" sx={{ minWidth: 120 }}>
            ...
            >
              <MenuItem value="bug" sx={{ color: "#EC5962", fontWeight: 600 }}>
                bug
              </MenuItem>
              <MenuItem
                value="feature"
                sx={{ color: "#007DFA", fontWeight: 600 }}
              >
                feature
              </MenuItem>
            </Select>
      ...
      <DialogContent sx={{ pt: 2 }}>
        ...
        <Box
          sx={{ display: "flex", justifyContent: "flex-start", mb: 3, gap: 2 }}
        >
          <Box
            sx={{
              display: "flex",
          ...
          >
            { availableUsers.length > 0 ? (
              ...
              <Typography
                sx={{ fontWeight: 400, fontSize: "16.28px", color: "red" }}
              >
